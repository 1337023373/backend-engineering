...
-   [一、框架前言](#一框架前言)
    -   [1. 总体技术体系](#1-总体技术体系)
    -   [2. 框架概念和理解](#2-框架概念和理解)
-   [二、Spring Framework简介](#二Spring-Framework简介)
    -   [1. Spring 和 SpringFramework](#1-Spring-和-SpringFramework)
    -   [2. SpringFramework主要功能模块](#2-SpringFramework主要功能模块)
    -   [3. SpringFramework 主要优势](#3-SpringFramework-主要优势)
-   [三、Spring IoC 容器概念](#三Spring-IoC-容器概念)
    -   [1. 组件和组件管理概念](#1-组件和组件管理概念)
    -   [2. Spring IoC容器和容器实现](#2-Spring-IoC容器和容器实现)
    -   [3. Spring IoC / DI概念总结](#3-Spring-IoC--DI概念总结)
-   [四、Spring IoC / DI 实现](#四Spring-IoC--DI-实现)
    -   [1.  Spring IoC / DI 实现步骤](#1--Spring-IoC--DI-实现步骤)
    -   [2. 基于XML方式管理Bean](#2-基于XML方式管理Bean)
        -   [2.1 实验一： 声明配置文件和创建容器](#21-实验一-声明配置文件和创建容器)
        -   [2.2 实验二： 获取Bean](#22-实验二-获取Bean)
        -   [2.3 实验三： Bean属性赋值：setter注入](#23-实验三-Bean属性赋值setter注入)
        -   [2.4 实验四： Bean属性赋值：引用其他Bean](#24-实验四-Bean属性赋值引用其他Bean)
        -   [2.5 实验五： Bean 属性赋值：内部Bean声明(了解)](#25-实验五-Bean-属性赋值内部Bean声明了解)
        -   [2.6 实验六： Bean 属性赋值：引入外部Properties配置参数](#26-实验六-Bean-属性赋值引入外部Properties配置参数)
        -   [2.7 实验七： 高级特性：FactoryBean特性](#27-实验七-高级特性FactoryBean特性)
        -   [2.8 实验八： 高级特性：Bean的作用域](#28-实验八-高级特性Bean的作用域)
        -   [2.9 实验九： 高级特性：Bean的生命周期](#29-实验九-高级特性Bean的生命周期)
    -   [3. 基于 注解 方式管理 Bean](#3-基于-注解-方式管理-Bean)
        -   [3.1 实验一： Bean注解标记和扫描(IoC)](#31-实验一-Bean注解标记和扫描IoC)
        -   [3.2 实验二： Bean属性赋值：引用类型自动装配(DI)](#32-实验二-Bean属性赋值引用类型自动装配DI)
        -   [3.3 实验三： Bean属性赋值：基本类型属性赋值(DI)](#33-实验三-Bean属性赋值基本类型属性赋值DI)
    -   [4. 基于 配置类 方式管理 Bean](#4-基于-配置类-方式管理-Bean)
        -   [4.1 完全注解开发理解](#41-完全注解开发理解)
        -   [4.2 实验一：配置类和扫描注解](#42-实验一配置类和扫描注解)
        -   [4.3 实验二：@Bean定义组件](#43-实验二Bean定义组件)
        -   [4.4 实验三：高级特性：@Bean注解细节](#44-实验三高级特性Bean注解细节)
        -   [4.5 实验四：高级特性：@Import扩展](#45-实验四高级特性Import扩展)
        -   [4.6 实验五：高级特性：@Conditional扩展(了解)](#46-实验五高级特性Conditional扩展了解)
    -   [5. 三种配置方式总结](#5-三种配置方式总结)
        -   [5.1 XML方式配置总结](#51-XML方式配置总结)
        -   [5.2 XML+注解方式配置总结](#52-XML注解方式配置总结)
        -   [5.3 完全注解方式配置总结](#53-完全注解方式配置总结)
    -   [6. 整合Spring5-Test5搭建测试环境](#6-整合Spring5-Test5搭建测试环境)
-   [五、Spring AOP面向切面编程](#五Spring-AOP面向切面编程)
    -   [1. 场景设定和问题复现](#1-场景设定和问题复现)
    -   [2. 解决技术代理模式](#2-解决技术代理模式)
    -   [3. 面向切面编程思维（AOP）](#3-面向切面编程思维AOP)
    -   [4. Spring AOP框架介绍和关系梳理](#4-Spring-AOP框架介绍和关系梳理)
    -   [5. Spring AOP基于注解方式实现和细节](#5-Spring-AOP基于注解方式实现和细节)
        -   [5.1 Spring AOP底层技术组成](#51-Spring-AOP底层技术组成)
        -   [5.2 初步实现](#52-初步实现)
        -   [5.3 获取通知细节信息](#53-获取通知细节信息)
        -   [5.4 切点表达式语法](#54-切点表达式语法)
        -   [5.5 重用（提取）切点表达式](#55-重用提取切点表达式)
        -   [5.6 环绕通知](#56-环绕通知)
        -   [5.7 切面优先级设置](#57-切面优先级设置)
        -   [5.8 CGLib动态代理生效](#58-CGLib动态代理生效)
        -   [5.9 注解实现小结](#59-注解实现小结)
    -   [6. SpringAOP基于XML方式实现(了解)准备工作](#6-SpringAOP基于XML方式实现了解准备工作)
    -   [7. SpringAOP对获取Bean的影响理解](#7-SpringAOP对获取Bean的影响理解)
        -   [7.1 根据类型装配 bean](#71-根据类型装配-bean)
        -   [7.2 使用总结](#72-使用总结)
-   [六、Spring 声明式事务](#六Spring-声明式事务)
    -   [1. JdbcTemplate使用](#1-JdbcTemplate使用)
    -   [2. 声明式事务概念](#2-声明式事务概念)
        -   [2.1 编程式事务](#21-编程式事务)
        -   [2.2 声明式事务](#22-声明式事务)
        -   [2.3 Spring事务管理器](#23-Spring事务管理器)
    -   [3. 基于注解的声明式事务](#3-基于注解的声明式事务)
        -   [3.1 准备工作](#31-准备工作)
        -   [3.2 基本事务控制](#32-基本事务控制)
        -   [3.3 事务属性：只读](#33-事务属性只读)
        -   [3.4 事务属性：超时时间](#34-事务属性超时时间)
        -   [3.5 事务属性：事务异常](#35-事务属性事务异常)
        -   [3.6 事务属性：事务隔离级别](#36-事务属性事务隔离级别)
        -   [3.7 事务属性：事务传播行为](#37-事务属性事务传播行为)
...
-   分布式架构
    ...
    ![](image/image_8Kl2EAU20K.png)

    分布式架构，项目主要应用技术框架：SpringBoot , SpringCloud , 中间件等

### 2. 框架概念和理解

框架( Framework )是一个集成了基本结构、规范、设计模式、编程语言和程序库等基础组件的软件系统，它可以用来构建更高级别的应用程序。框架的设计和实现旨在解决**特定领域**中的**常见问题**，帮助开发人员更高效、更稳定地实现软件开发目标。

![](image/image_zRRJPWQZSv.png)
...
-   **1.1 什么是组件？**
    ...
    ![](image/image_2wTZ1txGkv.png)

    整个项目就是由各种组件搭建而成的：

    ![](image/image_M2O5qkEH2G.png)
-   **1.2 我们的期待？**
    -   有人替我们创建组件的对象
    -   有人帮我们保存组件的对象
    -   有人帮助我们自动组装 
...
-   **2.2 SpringIoC容器介绍**
    ...
    Spring IoC 容器，负责实例化、配置和组装 bean（组件）。容器通过读取配置元数据来获取有关要实例化、配置和组装组件的指令。配置元数据以 **XML、Java 注释**或\*\* Java 代码形式\*\*表现。它允许表达组成应用程序的组件以及这些组件之间丰富的相互依赖关系。

    ![](image/image_DRyc99Tj3i.png)
...
-   **2.4 SpringIoC容器管理配置方式**
    ...
    3.  Java配置类方式：从Spring 3.0版本开始提供支持，通过Java类来定义Bean、Bean之间的依赖关系和配置信息，从而代替XML配置文件的方式。Java配置类是一种使用Java编写配置信息的方式，通过@Configuration、@Bean等注解来实现Bean和依赖关系的配置。
        配置方式的使用场景不同，为了更多体验每种方式，SSM期间，我们使用**XML+注解方式为主**。SpringBoot期间，我们使用**配置类+注解方式为主**！
...
1.  FactoryBean简介
    ...
    `FactoryBean` 接口是Spring IoC容器实例化逻辑的可插拔性特点。
...
![image](二、Spring企业开发核心框架.assets/image.png)
...
1.  **代理模式**
    ...
    二十三种设计模式中的一种，属于结构型模式。它的作用就是通过提供一个代理类，让我们在调用目标方法的时候，**不再是直接对目标方法进行调用**，而是**通过代理类间接调用**。让不属于目标方法核心逻辑的代码从目标方法中剥离出来——解耦。调用目标方法时先调用代理对象的方法，减少对目标方法的调用和打扰，同时让附加功能能够集中在一起也有利于统一维护。
...
![](image/image_flHAEvDMdm.png)

对没实现接口的类应用切面

![](image/image_IVnmsLNLA0.png)
